 ...
 * Description: This C++ program is to implement Dungeon game.
*********************************************************************/
#include "main.h"
...
// game mode flag
bool isCreatureMove = false;
bool isBattle = false;
bool isGameLoad = false; // handle load Exit
...
int main(int argc, char** argv)
{
	...
	while (true) {
		std::cout << "**********************************************" << std::endl;
		std::cout << "* Game Mode: [Basic], [Battle], [Custom]     *" << std::endl;
		std::cout << "**********************************************" << std::endl;
		std::cout << "** Basic Mode has different difficulty level *" << std::endl;
		std::cout << "** Battle Mode can kill the creatures        *" << std::endl;
		std::cout << "** Custom Mode let you customize map size    *" << std::endl;
		std::cout << "**********************************************" << std::endl;
		std::cout << "Enter Game Mode: ";
		std::cin >> gameMode;
		system("CLS");
		if (gameMode != "Basic" && gameMode != "Battle" && gameMode != "Custom") {
			std::cout << "Illegal Game Mode, please type again." << std::endl;
		}
		else {
			while (gameMode == "Custom") {
			...
			if (gameMode == "Basic") {
				loadMap(true); // choose difficulty level
				if (!isGameLoad) {
					system("CLS");
					continue; // to choose game mode again
				}
			}
	...
	while (!gKeyState[ValidInput::EESC]) {
		...
		if (gKeyState[ValidInput::ESave]) {
			...
			loadMap(false); // mark not basic game mode
...
void drawInfo(void)
{
	...
	if (isBattle) {
		for (int i = 0; i < gCreatures.size(); i++) {
			std::cout << gCreatures[i]->getIcon() << " has " << gCreatures[i]->getHP() << " hp" << std::endl;
		}
	}
	...
	for (int i = 0; i < gItems.size(); i++) {
		...
		std::cout << "| Every time you step on a " << item << " " << icon << ", the hero gets " << gItems[i]->getExpAmount() << " ";
		std::cout << effect;
	}
...
void update(bool key[])
{
	...
	for (int i = 0; i < gCreatures.size(); i++) {
		// if creature are alive
		if (gCreatures[i]->getDispIcon() != 'X') {
			gCreatures[i]->update(gHero); // attack hero

			if (isBattle) { // in battle game mode
				gCreatures[i]->kill(gHero); // hero can attack creature
			}
		}
		if (isCreatureMove) { // in hard game mode
			gCreatures[i]->move(gHero.getPos());
		}
	}
...
void saveMap() {
	...
	while (true) {
		...
		if (input.compare("Exit") == 0)
			return;
		system("CLS");
		if (input.compare("Easy") == 0 || input.compare("Normal") == 0 || input.compare("Hard") == 0) {
			std::cout << "Illegal file name, please name another." << std::endl;
		}
		else {
			break;
		}
	...
	std::ofstream oStream("saveMap/" + input + ".txt");
...
void loadMap(bool isBasic) {
	...
	while (true) {
		if (isBasic) {
			std::cout << "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" << std::endl;
			std::cout << "+ Game Level: [Easy], [Normal], [Hard]                   +" << std::endl;
			std::cout << "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" << std::endl;
			std::cout << "++++ Easy Level has a map with four walls only           +" << std::endl;
			std::cout << "++++ Normal Level has a map as a maze                    +" << std::endl;
			std::cout << "++++ Hard Level has a map with moving characters in maze +" << std::endl;
			std::cout << "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" << std::endl;
			std::cout << "Input certain level for loading or Exit to leave loading." << std::endl;
		}
		else {
			std::cout << "Input file name for loading or Exit to play new game." << std::endl;
		}
		...
		iStream.open("saveMap/" + input + ".txt");
		system("CLS");
		if (!iStream.is_open()) {
			std::cout << "Illegal, please type again." << std::endl;
		}
		else {
			std::cout << "Loading...ok!" << std::endl;
			std::cout << "Press Enter to start game!" << std::endl;

			// in hard difficulty game mode
			if (input == "Hard") {
				isCreatureMove = true;
				// creature can move towards hero
			}
